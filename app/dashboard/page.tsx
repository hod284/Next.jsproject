...
export default function DashboardPage()
{
    ...
    const fetchstate = async() =>{
        try
        {
            ...
            const response = await statsApi.getDash();
             console.log(' Stats API 응답:', response);
            if(response.success&& response.data)
            {
                   console.log(' Stats 데이터:', response.data);
  ...
  if (!stats) {
   ...
   // ✅ 총 매출 (이미 문자열 형태로 옴: "2,365,000")
    const totalSales = safeParse(stats.totalsales);
    
    // ✅ 차트 데이터 변환
    const monthlyChartData = stats.monthlysales.map(item => ({
        month: item.month,
        매출: safeParse(item.total_sales)
    }));

    const categoryChartData = stats.category.map(item => ({
        category: item.category,
        매출: safeParse(item.total_sales),
        주문수: safeParse(item.order_count)
    }));
   return (
                 ...
                 <div className="bg-white rounded-xl shadow-md p-6 border border-gray-100">
                     <div className="flex items-center justify-between">
                         <div>
                             ...
                             <p className="text-2xl font-bold text-gray-900">
                                 {stats.totaluser.toLocaleString()}명
                             </p>
                 ...
                 <div className="bg-white rounded-xl shadow-md p-6 border border-gray-100">
                     <div className="flex items-center justify-between">
                         <div>
                             ...
                             <p className="text-2xl font-bold text-gray-900">
                                 {stats.totalorders > 0 
                                     ? Math.floor(totalSales / stats.totalorders).toLocaleString()
                                     : 0}원
                             </p>
                             ...
                             {stats.monthlysales.map((item, index) => (
                                 <tr key={index} className="border-b border-gray-100 hover:bg-gray-50">
                                     ...
                                     <td className="py-3 px-4 text-sm text-gray-900 text-right font-medium">
                                         {safeParse(item.total_sales).toLocaleString()}원
                                     </td>
                                 </tr>
                             ))}
                         </tbody>
                             ...
                             {stats.category.map((item, index) => (
                                 <tr key={index} className="border-b border-gray-100 hover:bg-gray-50">
                                     ...
                                     <td className="py-3 px-4 text-sm text-gray-600 text-right">
                                         {safeParse(item.order_count).toLocaleString()}건
                                     </td>
                                     <td className="py-3 px-4 text-sm text-gray-900 text-right font-medium">
                                         {safeParse(item.total_sales).toLocaleString()}원
                                     </td>
